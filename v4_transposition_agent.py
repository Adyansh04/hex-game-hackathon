"""
Optimized competitive Hex agent for the AICA Game AI Platform.

Major improvements over the previous final_hex_agent.py
-------------------------------------------------------
1. Same proven strategy core: full-board one-move win/block checks, root two-ply
   threat guard, path-focused alpha-beta, and no-swap opening book.
2. Faster internal board representation: each 11x11 board is flattened to a
   1D list of 121 cells. Public Move objects still use [row, col].
3. Zobrist hashing + transposition table for alpha-beta search.
4. Per-move Dijkstra/path cache keyed by Zobrist hash and player.
5. 1D Dijkstra buffers are reused instead of allocating 2D arrays every call.
6. Optimized time checks: the recursive search polls the wall clock only every
   CHECK_INTERVAL nodes, with explicit checks around root-level expensive work.
7. Lightweight bridge-aware distance: already-formed bridges between own stones
   are treated as a cheap virtual connection in the distance estimate.
...
class HexAgent(Agent):
    ...
    def get_move(self, state: State) -> Move:
        ...
        winning_move = self._find_immediate_win(board, hash_key, player, legal_moves)
        if winning_move is not None:
            return self._make_move(player, winning_move)

        blocking_move = self._find_immediate_win(board, hash_key, opponent, legal_moves)
        ...
        try:
            for depth in range(1, self.MAX_SEARCH_DEPTH + 1):
                if time.perf_counter() >= deadline - 0.08:
                    break
                move, score = self._alpha_beta_root(board, hash_key, player, root_candidates, depth, deadline)
                if move is not None:
                    best_move = move
                    best_score = score
        ...
        if self.DEBUG:
            ...
            print(
                ...
                f"score={best_score:.2f} move={self._idx_to_pos(best_move)}"
            )
    ...
    def _has_player_won(self, board: list[int], player: int) -> bool:
        ...
        if player == PLAYER_0:
            for r in range(n):
                idx = r * n
                if board[idx] == player:
                    self._visited[idx] = mark
                    queue.append(idx)
            while queue:
                cur = queue.popleft()
                if self._col[cur] == n - 1:
                    return True
                for nxt in self._neighbors[cur]:
                    if self._visited[nxt] != mark and board[nxt] == player:
                        self._visited[nxt] = mark
                        queue.append(nxt)
        else:
            for c in range(n):
                idx = c
    ...
    def _find_immediate_win(self, board: list[int], hash_key: int, player: int, legal_moves: list[int]) -> int | None:
    ...
    def _count_immediate_wins(self, board: list[int], player: int, legal_moves: list[int] | None = None, cap: int = 2) -> int:
    ...
    def _rank_root_moves(
        ...
        for move in legal_moves:
            ...
            score -= self._root_danger_score_after_move(board, hash_key, move, player, deadline)
    ...
    def _root_danger_score_after_move(self, board: list[int], hash_key: int, move: int, player: int, deadline: float) -> float:
    ...
    def _alpha_beta(
        ...
        # Prefer TT best move if available.
        if entry is not None and entry.best_move in candidates:
            candidates.remove(entry.best_move)  # type: ignore[arg-type]
            candidates.insert(0, entry.best_move)  # type: ignore[arg-type]
        ...
        if turn == root_player:
            ...
            for move in candidates:
                ...
                board[move] = turn
                try:
                    child_score = self._alpha_beta(board, child_hash, self._other(turn), root_player, depth - 1, alpha, beta, deadline)
                finally:
                    board[move] = EMPTY
